...
void TankSpriteSet::Load(const std::string& assetsDir, const std::string& playerTag) {
    for (int level = 0; level < kLoadedLevels; ++level) {
        ...
        for (int d = 0; d < 4; ++d) {
            for (int f = 0; f < 2; ++f) {
                const std::string path = assetsDir + "sprites/tank_" + playerTag + "_" + levelTag + kDirNames[d] + "_" + std::to_string(f) + ".png";
